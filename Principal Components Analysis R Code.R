# Derek Higham
# MSDS 410 - Assignment 6
# Ames Data EDA

# Read in the csv file into an R data frame
my.path <- '/Users/derekhigham/Documents/school/MSDS_410/'
my.file <- paste(my.path, 'stock_portfolio.csv', sep='')
my.data <- read.csv(my.file, header=TRUE, stringsAsFactors=FALSE)
...
# Model excluding SLB and GS
model.3 <- lm(VV ~ AA+BAC+JPM+WFC+BHI+CVX+DOW+DPS+HON+HUN+KO+MMM+MPC+XOM+PEP, data=returns.df)
summary(model.3)
vif(model.3)
...
##### Backward Variable Selection and PCA #####
